...
for i in range(2 * k, n):
    # то же самое для третьего списка
    max_3[i] = max(max_3[i - 1], max_2[i - k] + data[i])
 # в конце выйдет максимум
print(max_3[n - 1])
...
# то же самое для правой границы
max_right = [-1] * n
# то же но наоборот
max_right[n - 1] = data[n - 1]
for i in range(1, n):
...
with open('27A_2903.txt') as file:
    ...
    for i in range(n):
        ...
        if count >= 10: min_sum = min(min_sum, sum_s - list_s[count - 10])
        # минимум суммы чисел и разница его индекса с минус 10 чтобы в разнице кол во чисел делящихся на 3 было ровно 10
