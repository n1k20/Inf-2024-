...
for count in range(n):
    if (sum_s + data[count]) > s:
        # просто перебираем с миинимальной суммы
        break
    sum_s += data[count]
...
with open('26_14959 (1).txt') as file:
...
with open('26_15341 (1).txt') as file:
    ...
    for i in range(counter):
        ...
        if number == 1:
            ...
            other_music.append((start, start + final))  # тоже самое
...
def check(start, final):
    ...
    for start_1, final_1 in favorite_music:
        if start_1 > final: return True
        if start < final_1 and final > start_1:  return False
used_other_music = []
...
for i in range(224466, 664422 + 1):
    if i % 5 == 0 and i % 7 == 0 and \
            i % 13 == 0 and i % (5 ** 2) != 0 \
            and i % (7 ** 2) != 0 and i % (13 ** 2) != 0 \
            and max_del(i) <= 10 ** 5:
        print(i, max_del(i))
